...
/* Read directory (copied from tool/common/fs.c, since that can't be part of this build)
 */
...
int inotify_watch(struct inotify *ino,const char *path) {
  ...
  int wd=inotify_add_watch(ino->fd,path,IN_CREATE|IN_ATTRIB|IN_MOVED_TO|IN_MODIFY);
  if (wd<0) return -1;
